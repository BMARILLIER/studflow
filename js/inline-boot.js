...
(function() {
    ...
    var ACTION_MAP = {
        ...
        'screen:flashcard':     function() { if (StudFlow.flashcards) StudFlow.flashcards.start(); },
        'screen:quiz':          function() { if (StudFlow.quiz) StudFlow.quiz.start(); },
        'pdf-result:flashcards': function() { if (StudFlow.flashcards) StudFlow.flashcards.start('imported'); },
        'pdf-result:quiz':      function() { if (StudFlow.quiz) StudFlow.quiz.launchQuiz('imported'); },
        'screen:coach':         function() { if (StudFlow.coach) StudFlow.coach.show(); },
        'screen:stress':        function() { if (StudFlow.stress) StudFlow.stress.show(); },
        'screen:focus':         function() { if (StudFlow.focus) StudFlow.focus.show(); },
        'screen:bacfrancais':   function() { if (StudFlow.bacfrancais) StudFlow.bacfrancais.show(); },
        ...
        'screen:planbac':       function() { if (StudFlow.planbac) StudFlow.planbac.show(); },
        'screen:urgence':       function() { if (StudFlow.urgence) StudFlow.urgence.show(); },
        'screen:confiance':     function() { if (StudFlow.confiance) StudFlow.confiance.show(); },
        'screen:generators':    function() { StudFlow.app.showScreen('generators'); },
        'screen:admin':         function() { StudFlow.app.showScreen('admin'); },
        'admin:switch-student': function() { if (StudFlow.roles) { StudFlow.roles.toggleStudentPreview(); StudFlow.app.showScreen('dashboard'); } },
        ...
        'auth:login':           function() { if (StudFlow.auth) StudFlow.auth.openModal('login'); },
        'auth:submit':          function() { if (StudFlow.auth) StudFlow.auth.handleSubmit(); },
        'auth:close':           function() { if (StudFlow.auth) StudFlow.auth.closeModal(); },
        'auth:toggle':          function() { if (StudFlow.auth) StudFlow.auth.handleToggle(); },
        'auth:export':          function() { if (StudFlow.auth) StudFlow.auth.exportData(); },
        'auth:delete-local':    function() { if (StudFlow.auth) StudFlow.auth.deleteLocalData(); },
        'fc:flip':              function() { if (StudFlow.flashcards) StudFlow.flashcards.flip(); },
        'fc:know':              function() { if (StudFlow.flashcards) StudFlow.flashcards.answer(true); },
        'fc:dont-know':         function() { if (StudFlow.flashcards) StudFlow.flashcards.answer(false); },
        'fc:easy':              function() { if (StudFlow.flashcards) StudFlow.flashcards.answer('easy'); },
        'fc:simplify':          function() { if (StudFlow.flashcards) StudFlow.flashcards.simplify(); },
        'fc:micro':             function() { if (StudFlow.flashcards.microExplain) StudFlow.flashcards.microExplain(); },
        'settings:calmToggle':  function(t) { if (StudFlow.calmMode) { StudFlow.calmMode.setManual(t.checked); } },
        'fc:shuffle':           function() { if (StudFlow.flashcards) StudFlow.flashcards.shuffleDeck(); },
        'fc:create-form':       function() { if (StudFlow.flashcards) StudFlow.flashcards.showCreateForm(); },
        'fc:create':            function() { if (StudFlow.flashcards) StudFlow.flashcards.createCard(); },
        'flashcards.startMode': function(p) { if (StudFlow.flashcards) StudFlow.flashcards.startMode(p); },
        'flashcards.showModePicker': function(p) { if (StudFlow.flashcards) StudFlow.flashcards.showModePicker(p || 'all'); },
        'quiz:simplify':        function() { if (StudFlow.quiz) StudFlow.quiz.simplify(); },
        'quiz:micro':           function() { if (StudFlow.quiz.microExplain) StudFlow.quiz.microExplain(); },
        ...
        'quiz:create-form':     function() { if (StudFlow.quiz) StudFlow.quiz.showCreateForm(); },
        'quiz:create':          function() { if (StudFlow.quiz) StudFlow.quiz.createQuestion(); },
        'quiz:launch-deck':     function(target) { if (StudFlow.quiz) { var deck = target.getAttribute('data-param') || 'all'; StudFlow.quiz.launchQuiz(deck); } },
        ...
        'breathing:toggle':     function() { if (StudFlow.breathing) StudFlow.breathing.toggle(); },
        'breathing:stop':       function() { if (StudFlow.breathing) StudFlow.breathing.stop(); StudFlow.app.showScreen('dashboard'); },
        'diagnostic:start':     function() { if (StudFlow.diagnostic) StudFlow.diagnostic.start(); },
        'feedback:show':        function() { if (StudFlow.feedback) StudFlow.feedback.show(); },
        'feedback:bug':         function() { if (StudFlow.feedback) { StudFlow.feedback.show(); setTimeout(function(){StudFlow.feedback.showQuickForm('bug');},100); } },
        'feedback:idea':        function() { if (StudFlow.feedback) { StudFlow.feedback.show(); setTimeout(function(){StudFlow.feedback.showQuickForm('idee');},100); } },
        'backup:export':        function() { if (StudFlow.backup) StudFlow.backup.exportData(); },
        'backup:import':        function() { if (StudFlow.backup) StudFlow.backup.importData(); },
        'errorlog:export':      function() { if (StudFlow.errorLog) StudFlow.errorLog.exportLog(); },
        ...
        'groq:clear':           function() {
            ...
            if (status) { status.className = 'api-status success'; status.textContent = 'Cle supprimee.'; }
        },
        ...
        'feedback:bug-then-show': function() { if (StudFlow.feedback) { StudFlow.feedback.showQuickForm('bug'); StudFlow.feedback.show(); } },
